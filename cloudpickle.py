...
Python3 compatability by Target Holding BV
email: jean-paul.van.oosten@target-holding.nl
...
class CloudPickler(Pickler):
    ...
    def save_codeobject(self, obj, pack=struct.pack):
        ...
        if printSerialization:
            print("Saving code object:", obj)
        args = (
            obj.co_argcount, obj.co_nlocals,
        ...
        if six.PY3:
            # In python3 there is an extra kwonlyargcount argument to CodeType.
            args = list(args)
            args.insert(1, obj.co_kwonlyargcount)
            args = tuple(args)
        self.save_reduce(_make_code_obj, args, obj=obj)
    ...
    def save_function_tuple(self, func, forced_imports):
        ...
        if printSerialization:
            print("Serializing", func, "with:", code)
        
        save(_fill_function)  # skeleton function updater
        #save(print)  # skeleton function updater
        write(pickle.MARK)    # beginning of tuple that _fill_function expects 

        # create a skeleton function object and memoize it
        save(_make_skel_func)
        save((code, len(closure), base_globals))
        write(pickle.REDUCE) # applies _make_skel_func on the above tuple
        self.memoize(func)
    ...
    def extract_func_data(self, func):
        ...
        #print("Exctracted func_data:", code, f_globals, defaults, closure, dct, base_globals)
    ...
    def save_instancemethod(self, obj):
        ...
        if six.PY2:
            self.save_reduce(types.MethodType, (obj.__func__, obj.__self__,
                obj.__self__.__class__), obj=obj)
        else:
            self.save_function(_make_method)
            self.write(pickle.MARK)
            self.save_function(obj.__func__)
            self.save(obj.__self__)
            self.write(pickle.TUPLE)
            self.write(pickle.REDUCE)
    ...
    if six.PY2:
        ...
        try:
            ...
            import warnings
            warnings.warn("python versions < 2.6 are not supported.")
    ...
    if six.PY3:
        dispatch[io._io.TextIOWrapper] = save_file
    else:
...
def_protocol = 4 if six.PY3 else 2
def dump(obj, file, protocol=def_protocol):
    CloudPickler(file, protocol).dump(obj)

def dumps(obj, protocol=def_protocol):
    file = io.BytesIO()
...
def env_vars_load(env_dct):
    ...
    error_msg('Loaded Environment variables {}'.format(list(env_dct.keys())), logging.DEBUG)      
...
def _make_code_obj(*args):
    """Required in python3. Apparently, we cannot just pass everything directly
    to CodeType."""
